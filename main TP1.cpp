...
void lista_kg(string nom_prod[],string nom_cliente[],
             ...
             int cant_cliente,int cant_prod)
	{
	    ...
	    for(int i{0};i < cant_cliente;i++){
	        ...
	        for(int j{0};j < cant_prod;j++){
	            if(kg[i][j] > 13000){
	                //nombrar productos con mas de 13mil kgr...
	                cout << nom_prod[j] <<'\t';
	                cont[i]+= 1;
	            }
	        }
...
void lista_cliente_variedad(string nom_prod[],string nom_cliente[],int cod_cliente,
                           int list_var[],int seg[],float km[][cod_prod])
	{
	    cout << endl << "Pto. 2)" << endl;
...
void mayor_cant_entrega(string nom_prod[],string nom_cliente[],int cant_entregas[][cod_prod], float cant_km[][cod_prod],int cod_cliente,int seguimiento[],int cont_cliente[])
	{
	    cout << endl << "Pto. 3)" << endl;
...
int main()
{
    ...
    //peso total de cada viaje

    int cont_cliente[cod_cliente]{0};
    //cuenta cantidad de tipos de elemento que superan los 13.000
    ...
    lista_kg(nom_prod,nom_cliente,cant_peso,cont_cliente,cod_cliente,cod_prod);
    lista_cliente_variedad(nom_prod,nom_cliente,cod_cliente,cont_cliente,seguimiento,cant_km);
